<div align="center">
  <img alt="nexi" height="200px" src="./img/nexi-logo.png">
  <h1>Nexi AI</h1>
...
- [What is Nexi?](#what-is-nexi)
- [What are some use cases?](#what-are-some-use-cases)
- [Highlights](#highlights)
- [How to get started](#how-to-get-started)
  - [Generate Secret Keys](#generate-secret-keys)
  - [Deploy Nexi locally with Docker (simplest way)](#deploy-nexi-locally-with-docker-simplest-way)
  - [Deploy Nexi on a remote server](#deploy-nexi-on-a-remote-server)
...
- **⚽️ Football analysis**: Imagine a team of agents where one scours the web for the latest Premier League news, and another analyzes the data to write insightful reports on each team's performance in the new season.
...
- **Integrate Nexi with external application**: Use Nexi's public API endpoints to interact with your teams.
- **Easy Deployment**: Deploy Nexi effortlessly using Docker.
...
### Deploy Nexi locally with Docker (simplest way)

[Get up and started within minutes on your local machine.](./local-deployment.md)

### Deploy Nexi on a remote server

[Deploy Nexi on your remote server.](./deployment.md)
...
Skills are abilities that you can equip your agents with to interact with the world. For example, you can provide your agent with the skill to check the current weather condition or search the web for the latest news. By default, Nexi provides three skills:
...
If your skill involves performing an HTTP request to fetch or update data, using skill definitions is the simplest approach. In Nexi, start by navigating to the 'Skills' tab and clicking the 'Add Skill' button. You will then be prompted to provide the skill definition, which instructs your agent on how to execute the specific skill. This definition should be structured as follows:
...
Open source models are becoming cheaper and easier to run, and some even match the performance of closed models. You might prefer using them for their privacy and cost benefits. If you are running Nexi locally and want to use open source models, I would recommend Ollama for its ease of use.
...
3. **Configure your agents:**
   ...
   - By default, Nexi will run on `http://host.docker.internal:11434`, which maps to `https://localhost:11434`. This setup allows Nexi to communicate with the default Ollama host. If your setup uses a different host, specify the new host in the 'Base URL' input field.
...
Log into Nexi using the email and password you set during the installation step.
...
Congratulations! You’ve successfully built and communicated with your first multi-agent team on Nexi.
...
Nexi is licensed under the terms of the MIT license.
